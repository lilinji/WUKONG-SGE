...
#		getpasswd = list(itertools.chain([v.get('passwd') if not v.get('skip', False) else [] for v in self.config.itervalues()]))	     
		...
		allhosts = [v.get('hosts') if not v.get('skip', False) else [] for v in self.config.itervalues()]
			...
			passwd = self.config.get('compute').get('passwd')
			#self.cmd_helper.run_local('rm -rf /root/.ssh/known_hosts')
			#self.cmd_helper.run_local('./ssh_expect.sh root {0} {1}'.format(passwd, h))
	...
	def install_master_package(self):
		...
		self.cmd_helper.run_local('sleep 6')
		### get json list 
		...
		GLOBAL_IP = '''127.0.0.1   localhost localhost.localdomain localhost4 localhost4.localdomain4
::1         localhost localhost.localdomain localhost6 localhost6.localdomain6'''
		self.cmd_helper.run_local('hostname  {0}'.format(h))
		self.cmd_helper.run_local('sed -i  \'2a HOSTNAME={0}\' /etc/sysconfig/network'.format(h))
		self.cmd_helper.run_local('sed -i  \'2d\' /etc/sysconfig/network')
		hostslist=GLOBAL_IP+'\n'+n+'\t'+h+'\n'
		#print  hostslist
		...
		self.cmd_helper.run_local('wget -O /etc/yum.repos.d/CentOS-Base.repo http://mirrors.aliyun.com/repo/Centos-6.repo')
...
h_vmem                INFINITY'''
		...
		for r, v in self.iter_roles():
			...
			for n in (v.get('hosts')):
				...
				#########
				self.cmd_helper.run_local('ssh {0} \'hostname  {1}{2}\''.format(n, r, m))
				self.cmd_helper.run_local('ssh {0} \"sed -i  \'2a HOSTNAME={1}{2}\' /etc/sysconfig/network \"'.format(n, r, m))
				self.cmd_helper.run_local('ssh {0}   \"sed -i  \'2d\' /etc/sysconfig/network \"'.format(n, r, m))
				####Chang Compute Name
				passwd = self.config.get('compute').get('passwd')
